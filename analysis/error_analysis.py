...
def load_reference(tc_name):
    ...
    ref_file = Path('outputs_archive/reference_solutions') / f"{tc_name}_reference.npz"
    if not ref_file.exists():
        raise FileNotFoundError(f"Run analysis/reference_solver.py first. Missing: {ref_file}")
    d = np.load(ref_file)
    return d['u'], d['x'], d['t']
...
def run_error_analysis():
    ...
    for tc in test_cases:
        ...
        for model_label, suffix in [('PINN', 'pinn'), ('CAN-PINN', 'canpinn')]:
            filepath = Path('outputs_archive') / f"{prefix}_{suffix}_solution.npz"
            if not filepath.exists():
                print(f"  ✗ {model_label}: missing {filepath}")
                print(f"      → Run: python training/train_improved_allen_cahn.py")
                any_missing = True
                continue
    ...
    if any_missing:
        ...
        print("     python training/train_improved_allen_cahn.py")
        print("  Then re-run this script:")
        print("     python analysis/error_analysis.py")
